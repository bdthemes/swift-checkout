			...
			<?php
            // Display the total with shipping if shipping method is selected
            if (function_exists('WC') && !WC()->cart->is_empty()) {
                // Check if a shipping method is chosen
                $chosen_methods = WC()->session->get('chosen_shipping_methods');
                if (!empty($chosen_methods) && isset($chosen_methods[0])) {
                    echo '<div class="cart-total-with-shipping">';
                    echo '<div class="cart-total-with-shipping-label">' . esc_html__('Total (with shipping):', 'swift-checkout') . '</div>';
                    echo '<div class="cart-total-with-shipping-value">' . wp_kses_post(WC()->cart->get_total()) . '</div>';
                    echo '</div>';
                }
            }
            ?>